...
def load(
  ...
  try:
    ...
    # Candidate weight filenames in priority order:
    # 1. model.safetensors (HF default for new repos)
    # 2. pytorch_model.bin (legacy format)
    _WEIGHT_CANDIDATES = ["model.safetensors", "pytorch_model.bin"]
    ...
    if checkpoint_path is None:
      try:
        ...
        for fname in _WEIGHT_CANDIDATES:
          candidate = os.path.join(base_path, model_type, fname)
          if os.path.exists(candidate):
            checkpoint_file = candidate
            break
        if checkpoint_file is None:
          raise FileNotFoundError(
            f"No weight file found in {os.path.join(base_path, model_type)}. "
            f"Tried: {_WEIGHT_CANDIDATES}"
          )
      ...
      if os.path.isdir(checkpoint_file):
        for fname in _WEIGHT_CANDIDATES:
          for sub in [os.path.join(checkpoint_file, model_type, fname),
                      os.path.join(checkpoint_file, fname)]:
