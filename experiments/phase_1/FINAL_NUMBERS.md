...
> **Relocated 2026-08-26** from `experiments/FINAL_NUMBERS.md` to `experiments/phase_1/`. **Still the
> authoritative Phase 1 ledger and still wins any dispute with `phase_1_results.md`.** Every relative
> path in Section 9 (`tier0_instrument/…`, `tier4_value/…`, `tier5_positive/…`, `core/metrics.py`)
> resolves correctly from this file's new location — those directories moved with it. The two paths
> that did change are marked in place: `tier2_audit/` is now `_archive/quarantine/tier2_audit/`, and
> the Section 11 reference to Phase 2's array store is now `../phase_2/`.
...
| `_archive/quarantine/tier2_audit/instrument_c_d.py` (was `tier2_audit/…`) | modified — `get_metrics` returned `negfrac` as `negeig` | backup `.chunk0_backup` |
