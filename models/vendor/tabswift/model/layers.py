...
class MultiheadAttention(nn.MultiheadAttention):
    ...
    This extends PyTorch's MultiheadAttention
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.0, use_headwise_gate: bool = False, use_elementwise_gate: bool = False):
    ...
    def forward(
        ...
        return multi_head_attention_forward(
            ...
            gate_proj_weight=self.attn_gate.weight if self.use_headwise_gate or self.use_elementwise_gate else None,
            gate_proj_bias=self.attn_gate.bias if self.use_headwise_gate or self.use_elementwise_gate else None,
        )
...
class MultiheadAttentionBlock(nn.TransformerEncoderLayer):
    ...
    def __init__(
        ...
        del self.self_attn
        self.attn = MultiheadAttention(d_model, nhead, dropout, use_headwise_gate=use_headwise_gate, use_elementwise_gate=use_elementwise_gate  )
        self.init_weights()
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            attn = self._attn_block(self.norm1(q), self.norm1(k), self.norm1(v), key_padding_mask, attn_mask)
            x = x + attn
            x = x + self._ff_block(self.norm2(x))
        else:
            # Post-norm: normalize after attention and FFN
            attn = self._attn_block(q, k, v, key_padding_mask, attn_mask)
            x = self.norm1(x + attn)
    ...
    def _attn_block(
        ...
        attn = self.attn(q, k, v, key_padding_mask, attn_mask)
