...
# [VENDOR PATCH V1] scikit-learn >= 1.6 removed BaseEstimator._validate_data in
# favour of the module-level validate_data(estimator, X, y, ...). Upstream
# TabSwift still calls the removed method, so every fit() raises AttributeError
# on the scikit-learn installed here. Patched locally rather than by
# monkeypatching sklearn.base globally (which would change input validation for
# every estimator in the process, including the other two foundation models).
# Behaviour-preserving: validate_data() is the same routine under a new name and
# still sets n_features_in_ / feature_names_in_ on the estimator.
# See models/vendor/PATCHES.md.
...
def _validate_data_compat(estimator, X="no_validation", y="no_validation", **kwargs):
    """validate_data() on new scikit-learn, _validate_data() on old."""
    if _sk_validate_data is not None:
        return _sk_validate_data(estimator, X, y, **kwargs)
...
class UniqueFeatureFilter(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        self : object
            Returns self.
        """
        X = _validate_data_compat(self, X)
    ...
    def transform(self, X):
        ...
        X = _validate_data_compat(self, X, reset=False)
...
class PreprocessingPipeline(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        """
        check_is_fitted(self)
        X = _validate_data_compat(self, X, reset=False, copy=True)
        # Standard scaling
...
class FeatureShuffler:
    ...
    def shuffle(self, n_estimators: int) -> List[np.ndarray]:
        ...
        if self.method == "none" or n_estimators == 1:
            ...
            # [VENDOR PATCH V4] CHANGES NUMERICS. Upstream is missing this
            # `return`, so the identity pattern assigned above is immediately
            # overwritten by the `elif self.method == "latin"` branch below and a
            # latin-square permutation is applied even when a single ensemble
            # member was requested. TabICL's equivalent shuffler does return
            # here (site-packages/tabicl/_sklearn/preprocessing.py:816-819), which
            # is what makes the two libraries disagree. Without this, a
            # single-member TabSwift run silently reorders the caller's columns,
            # so per-feature attribution is misattributed.
            # See models/vendor/PATCHES.md.
...
class EnsembleGenerator(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self : object
            Fitted generator.
        """
        _validate_data_compat(self, X, y)
