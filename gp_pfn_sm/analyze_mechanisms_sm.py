...
def main():
    ...
    checkpoint_path = os.path.join(script_dir, "checkpoints/spectral_mixture.pt")
    results_dir = os.path.join(script_dir, "results/mechanistic_plots_sm")
    json_summary_path = os.path.join(script_dir, "results/mechanistic_summary.json")
    ...
    for l in range(num_layers):
        ...
        for idx, (name, coords) in enumerate(methods):
            ax = axes[idx]
            ax.scatter(coords[labels == 0, 0], coords[labels == 0, 1], alpha=0.5, label='ID (SM GP)', color='#1f77b4', s=4)
            ax.scatter(coords[labels == 1, 0], coords[labels == 1, 1], alpha=0.5, label='OOD (High-Freq)', color='#ff7f0e', s=4)
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(range(num_layers), intrinsic_dim_id, marker='o', label='ID (SM GP)', linewidth=2.0, color='#1f77b4')
    plt.plot(range(num_layers), intrinsic_dim_ood, marker='s', label='OOD (High-Freq)', linewidth=2.0, color='#ff7f0e')
    plt.xlabel("Layer Depth")
    ...
    plt.plot(layers_range, attn_entropy_id_means, marker='o', label='ID (SM GP)', color='#1f77b4', linewidth=2.0)
    plt.fill_between(
        layers_range,
    ...
    plt.plot(layers_range, attn_entropy_ood_means, marker='s', label='OOD (High-Freq)', color='#ff7f0e', linewidth=2.0)
    plt.fill_between(
        layers_range,
    ...
    plt.plot(update_layers, update_id_means, marker='o', label='ID (SM GP)', color='#1f77b4', linewidth=2.0)
    plt.plot(update_layers, update_ood_means, marker='s', label='OOD (High-Freq)', color='#ff7f0e', linewidth=2.0)
