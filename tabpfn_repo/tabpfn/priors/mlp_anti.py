...
class AntiLinearLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        A += mask0 * (P > 0).float()
        A += mask1 * torch.sign(P)
        A += mask2 * torch.sin(P)
        A += mask3 * torch.cos(P)
        A += mask4 * torch.abs(P)

        out = A.sum(dim=-2) # sum over in_features
...
def get_batch_anti(batch_size, seq_len, num_features, hyperparameters, device=default_device, num_outputs=1, sampling='normal'
              , epoch=None, **kwargs):
