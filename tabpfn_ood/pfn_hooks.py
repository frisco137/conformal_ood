...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tabpfn_repo')))
...
class PFNHookManager:
    ...
    """
    def __init__(self, checkpoint_path: str, filename: str, device: str = "cpu"):
        self.device = device
        self.checkpoint_path = checkpoint_path

        # Load the model
        print(f"Loading TabPFN from {checkpoint_path}...")
        # loaded_data = torch.load(checkpoint_path, map_location=device, weights_only=False)
        tup, config_sample = load_model_only_inference(path=checkpoint_path, filename=filename, device=device)
        self.model = tup[2]
        ...
        self.attention_maps_ctoc: Dict[int, List[torch.Tensor]] = {}
        self.attention_maps_qtoc: Dict[int, List[torch.Tensor]] = {}
        self.hooks: List[Any] = []
    ...
    def _register_hooks(self):
        ...
        for idx, layer in enumerate(self.model.transformer_encoder.layers):
            ...
            self.attention_maps_ctoc[idx] = []
            self.attention_maps_qtoc[idx] = []
            ...
            def make_layer_hook(l_idx: int):
                def layer_hook(module: nn.Module, input_args: Any, output_val: torch.Tensor):
                    ...
                    # We compute sequence mean immediately (reduce dim 0) to save >140GB of RAM!
                    self.activations[l_idx].append(output_val.detach().mean(dim=0).cpu())
                return layer_hook
            ...
            def make_sa_hook(l_idx: int):
                ...
                def sa_hook(module: nn.Module, input_args: Any, output_val: Tuple[torch.Tensor, torch.Tensor]):
                    ...
                    # attn_output_weights has shape: [batch_size, query_len, seq_len]
                    if isinstance(output_val, tuple) and len(output_val) > 1 and output_val[1] is not None:
                        w = output_val[1].detach().cpu()
                        if state["is_ctoc"]:
                            self.attention_maps_ctoc[l_idx].append(w)
                            state["is_ctoc"] = False
                        else:
                            self.attention_maps_qtoc[l_idx].append(w)
                            state["is_ctoc"] = True
                return sa_hook
                    '''
                # SKIPPING SAVING ATTENTION MAPS for now to save >250GB of RAM!
                    pass
    ...
    def get_aggregated_features(self) -> Tuple[Dict[int, torch.Tensor], Dict[int, torch.Tensor], Dict[int, torch.Tensor]]:
        """
        ...
        - layer_attention_ctoc: Dict of {layer_idx: Tensor of shape (total_samples, ctx_len, ctx_len)}
        - layer_attention_qtoc: Dict of {layer_idx: Tensor of shape (total_samples, query_len, ctx_len)}
        """
        layer_activations: Dict[int, torch.Tensor] = {}
        layer_attention_ctoc: Dict[int, torch.Tensor] = {}
        layer_attention_qtoc: Dict[int, torch.Tensor] = {}

        for idx in self.activations:
            if len(self.activations[idx]) == 0:
                continue

            # Since we appended the mean(dim=0) inside the hook, lists contain shapes [batch_size, hidden_dim]
            # Concatenate along the batch dimension (now dim 0)
            layer_activations[idx] = torch.cat(self.activations[idx], dim=0)

            if len(self.attention_maps_ctoc[idx]) > 0:
                layer_attention_ctoc[idx] = torch.cat(self.attention_maps_ctoc[idx], dim=0)
            if len(self.attention_maps_qtoc[idx]) > 0:
                layer_attention_qtoc[idx] = torch.cat(self.attention_maps_qtoc[idx], dim=0)

        return layer_activations, layer_attention_ctoc, layer_attention_qtoc
