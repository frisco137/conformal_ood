...
class AntiLinearLayer(nn.Module):
    def __init__(self, in_features, out_features, device, bias=True):
        ...
        # Edge assignments to different activation functions (0 to 4)
        self.edge_activations = torch.randint(0, 5, (in_features, out_features), device=device)
    ...
    def forward(self, x):
        x_unsq = x.unsqueeze(-1) # (..., in_features, 1)
        w_unsq = self.weight.t() # (in_features, out_features)

        # View w to broadcast with x
        w_view = w_unsq.view(*[1 for _ in range(x.dim() - 1)], self.in_features, self.out_features)

        P = x_unsq * w_view # (..., in_features, out_features)

        mask0 = (self.edge_activations == 0)
        mask1 = (self.edge_activations == 1)
        mask2 = (self.edge_activations == 2)
        mask3 = (self.edge_activations == 3)
        mask4 = (self.edge_activations == 4)

        # apply activations per edge
        A = torch.zeros_like(P)
        A += mask0 * P  # Linear
        A += mask1 * torch.relu(P)  # ReLU
        A += mask2 * torch.tanh(P)  # Tanh
        A += mask3 * torch.sigmoid(P)  # Sigmoid
        A += mask4 * torch.nn.functional.elu(P)  # ELU

        # Scale over the sum to prevent variance explosion (fixes NaN errors)
        out = A.sum(dim=-2) / math.sqrt(self.in_features)
...
def get_batch_anti(batch_size, seq_len, num_features, hyperparameters, device="cpu", num_outputs=1, sampling='normal'
              , epoch=None, **kwargs):
    ...
    uninformative_ratio = hyperparameters.get('uninformative_ratio', 0.1)
    k = max(1, int(num_features * (1. - uninformative_ratio)))
    num_uninformative = num_features - k
    ...
    class MLP(torch.nn.Module):
        def __init__(self, hyperparameters):
            ...
            with torch.no_grad():
                ...
                if 'verbose' in hyperparameters and self.verbose:
                    print({_k : hyperparameters[_k] for _k in ['is_causal', 'num_causes', 'prior_mlp_hidden_dim'
                        , 'num_layers', 'noise_std', 'y_is_effect', 'pre_sample_weights', 'prior_mlp_dropout_prob'
                        , 'pre_sample_causes']})

                if self.is_causal:
                    self.prior_mlp_hidden_dim = max(self.prior_mlp_hidden_dim, num_outputs + 2 * k)
                else:
                    self.num_causes = k
                ...
                def generate_module(layer_idx, in_dim, out_dim):
                    ...
                    return [
                        nn.Sequential(*[
                            AntiLinearLayer(in_dim, out_dim, device=device)
                            , noise])
                    ]

                self.layers = [AntiLinearLayer(self.num_causes, self.prior_mlp_hidden_dim, device=device)]
                self.layers += [module for layer_idx in range(self.num_layers-1) for module in generate_module(layer_idx, self.prior_mlp_hidden_dim, self.prior_mlp_hidden_dim)]
                if not self.is_causal:
                    self.layers += generate_module(-1, self.prior_mlp_hidden_dim, num_outputs)
        ...
        def forward(self):
            ...
            if self.is_causal:
                ...
                if self.in_clique:
                    random_perm = random.randint(0, outputs_flat.shape[-1] - num_outputs - k) + torch.randperm(num_outputs + k, device=device)
                else:
                ...
                random_idx = random_perm[num_outputs:num_outputs + k]
            ...
            # Phase 2: Inject uninformative features
            if num_uninformative > 0:
                noise_cols = []
                for idx in range(num_uninformative):
                    col_idx = idx % k
                    col = x[:, :, col_idx].clone()
                    perm = torch.randperm(seq_len, device=device)
                    col = col[perm, :]
                    noise_cols.append(col.unsqueeze(2))
                noise_x = torch.cat(noise_cols, dim=2)
                x = torch.cat([x, noise_x], dim=2)

            if bool(torch.any(torch.isnan(x)).detach().cpu().numpy()) or bool(torch.any(torch.isnan(y)).detach().cpu().numpy()):
                print('(anti) Nan caught in MLP model x:', torch.isnan(x).sum(), ' y:', torch.isnan(y).sum())
                x[:] = 0.0
                y[:] = -100 # default ignore index for CE

            # Phase 3: Random Feature Space Rotation
            M = torch.randn(num_features, num_features, device=device)
