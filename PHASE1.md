...
*Compiled 2026-08-26 from `experiments/FINAL_NUMBERS.md` revision 2 plus Sections 10 and 11.*

> **What this supersedes.** This document replaces `FINAL_NUMBERS.md` as the reading entry point for
> Phase 1. `FINAL_NUMBERS.md` stays as the ledger — it holds per-seed values, the retraction table,
...
>
> **Not to be confused with** `experiments/phase_2/PHASE2.md`, which is Phase 2's own consolidated
> record. The root-level `PHASE2.md` beside this file is the Phase 2 companion to this document.
...
*Script:* `experiments/core/context.py`, `experiments/core/metrics.py`.

**Canonical metric definitions**, and the only implementations:
`experiments/core/metrics.py:3` (`asym`), `:13` (`negeig`), `:135` (`get_Q`, seeded).
...
> `experiments/_quarantine/tier2_audit/`. No number in this document comes from it. Two historical
...
*Script:* `experiments/tier0_instrument/chunk1_control_validation.py` → `chunk1_results.json`,
...
*Script:* `experiments/core/controls.py` (the map), `chunk1_control_validation.py` → `chunk1_results.json`.
...
> is `0.084498`. Fixed at source in `experiments/core/controls.py`; regression test at
> `experiments/core/test_controls.py` (22 tests). The table above is the corrected version and is
...
*Script:* `experiments/tier0_instrument/recompute_orphans.py` → `clarif_item4_6.json`, key `<tag>_r1`.
...
*Script:* `experiments/tier0_instrument/e11_wrapper_form.py` → `e11_wrapper_form.json`.
...
*Script:* `experiments/tier5_positive/e5_1_positive_control.py` → `e5_1_results.json`. CPU, 55 s.
...
*Script:* `experiments/tier4_value/e4_1_value_battery.py` → `e4_1_results.json`. 3 models × 5 seeds on
...
| **E5.2, E5.3, E5.4** | Not run as registered tests. See the E5.2 note below. |
...
### 9.3 What is quarantined

`experiments/_quarantine/`, with `__init__.py` raising on import. Manifest at
