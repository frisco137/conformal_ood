# Phase 2 — Downstream Consequences of the Audit
...
*Compiled 2026-08-26 from `experiments/phase_2/PHASE2.md` (Parts A–D) and the JSON records under
`experiments/phase_2/results/`.*

> **Naming.** This file is the root-level Phase 2 document. `experiments/phase_2/PHASE2.md` is a
> different file — the four-part consolidated measurement record this one is compiled from. Where
> they differ, that one is the record and this one is the reading entry point. **Where either
> disagrees with `experiments/FINAL_NUMBERS.md` on a Phase 1 number, the ledger wins** (see §7).
>
...
> `experiments/phase_2/LOCALISATION.md` §0 records the same confusion.
...
*Script:* `experiments/phase_2/src/chunk2_data.py` → `results/chunk2_datasets.json`,
...
|---|---|---|
| 1 | `phase_2/README.md:123` states **"The tier-0 file still carries the bug"** about `TargetedImitator.inner_jacobian`; `UNCERTAINTY_EXPERIMENT.md:908` and `PHASE2.md:2944` say "the tier-0 file is unchanged". | **STALE as of 2026-08-26.** The bug was fixed at source in `experiments/core/controls.py`; the three inline workarounds now call the corrected method. **Verified bit-identical** (max abs diff exactly `0.0` on all five seeds), so **no Phase 2 number changes.** A correction banner was added to `phase_2/README.md`; the three record documents are left as history. |
...
**This document and `PHASE1.md` are the source of truth for the paper.** Everything else becomes
history, and **nothing is deleted**:

| document | status |
|---|---|
| `experiments/FINAL_NUMBERS.md` | **stays as the Phase 1 ledger.** Holds per-seed values, the retraction table, and the full provenance index. **Wins any dispute with `PHASE1.md`.** |
| `experiments/phase_2/PHASE2.md` | **stays as the Phase 2 record**, Parts A–D. Wins any dispute with this file. |
| `experiments/phase_2/RESULTS.md`, `LOCALISATION.md`, `SEQUENTIAL.md`, `UNCERTAINTY_EXPERIMENT.md` | **stay as history.** They are the four source documents consolidated into `phase_2/PHASE2.md`; their content is unchanged there. Not to be cited directly by the paper. |
| `experiments/experiments.md` | **frozen** as a dated pre-registration. See `experiments/DEVIATIONS.md`. |
| `experiments/theory_and_claims.md` | **live**, amended by revision. |

**Recommendation, stated because the tree supports it:** keep all of the above. The four Phase 2
source documents cost nothing to retain and `phase_2/PHASE2.md` states explicitly that their content
is unchanged, so the audit trail from raw JSON → part document → consolidated record → this file is
complete and checkable at every hop. That trail is worth more than the tidiness of deleting them —
particularly given that **nine other cited documents in this project have already been lost entirely**
(`PHASE1.md` §9.2).
